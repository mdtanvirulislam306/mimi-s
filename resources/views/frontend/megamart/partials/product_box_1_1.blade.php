    ...
    <div class="p-2 p-md-3 text-left">
        ...
        <div class="fs-14 d-flex justify-content-center mb-2">
            ...
            @if ($product->auction_product == 1)
                <!-- Bid Amount -->
                <div class="">
                    <span class="fw-700 text-secondary">{{ single_price($product->starting_bid) }}</span>
                </div>
            @endif
        </div>
        <a href="javascript:void(0)"onclick="showAddToCartModal({{ $product->id }})"
                class="w-100 custom-btn aiz-p-hov-icon fs-13 fw-700 d-flex flex-column justify-content-center align-items-center @if (in_array($product->id, $cart_added)) active @endif">
           <span>
                    {{ translate('Add to Cart') }}
            </span>
