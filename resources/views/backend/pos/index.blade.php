            ...
            <div class="col-md">
                <div class="row gutters-5 mb-3">
                    ...
                    <div class="col-md-3 mb-2 mb-md-0">
                        <div class="form-group mb-0">
                            @php
                                $branch = \App\Models\BranchModel::find(auth()->user()->branch_id);
                                if ($branch) {
                                    echo '<select name="branch_id" class="form-control form-control-lg" onchange="filterProducts()">';
                                        echo '<option value="' . $branch->id . '">' . $branch->name . '</option>';
                                    echo '</select>';
                                }else{
                                    $branches = \App\Models\BranchModel::all();
                                    echo '<select name="branch_id" class="form-control form-control-lg" onchange="filterProducts()">';
                                    echo '<option value="">' . translate('Select Branch') . '</option>';
                ...
                <div class="card mb-3">
                    <div class="card-body">
                        <div class="d-flex border-bottom pb-3">
                            ...
                            <button type="button" class="btn btn-icon text-white btn-soft-dark ml-3 mr-0" data-target="#new-customer" data-toggle="modal">
    ...
    $.post('{{ route('pos.addToCart') }}',{_token:AIZ.data.csrf, stock_id:stock_id}, function(data){
        ...
        function loadMoreProduct(){
            ...
            if(products != null && products.links.next != null){
                ...
                $.get(products.links.next,{keyword:keyword, category:category, brand:brand,branch_id:branch_id}, function(data){
                    products = data;
                    setProductList(data);
                });
            }
