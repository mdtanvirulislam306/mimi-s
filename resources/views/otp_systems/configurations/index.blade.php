...
@section('content')
    <div class="row">
        ...
        <div class="col-lg-6">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 h6">{{translate('Smart Software BD Credential')}}</h5>
                </div>
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('update_credentials') }}" method="POST">
                        <input type="hidden" name="otp_method" value="smartsoftwarebd">
                        @csrf
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="SMART_SOFTWARE_BD_API_KEY">
                            <div class="col-lg-3">
                                <label class="col-from-label">{{translate('SMART SOFTWARE BD API KEY')}}</label>
                            </div>
                            <div class="col-lg-6">
                                <input type="text" class="form-control" name="SMART_SOFTWARE_BD_API_KEY" value="{{  env('SMART_SOFTWARE_BD_API_KEY') }}" placeholder="SMART SOFTWARE BD API KEY" required>
                            </div>
                        </div>
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="SMART_SOFTWARE_BD_SENDER_ID">
                            <div class="col-lg-3">
                                <label class="col-from-label">{{translate('SMART SOFTWARE BD SENDER ID')}}</label>
                            </div>
                            <div class="col-lg-6">
                                <input type="text" class="form-control" name="SMART_SOFTWARE_BD_SENDER_ID" value="{{  env('SMART_SOFTWARE_BD_SENDER_ID') }}" placeholder="SMART SOFTWARE BD SENDER ID" required>
                            </div>
                        </div>
                        <div class="form-group mb-0 text-right">
                            <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
